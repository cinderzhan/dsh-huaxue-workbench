...
export class WorkbenchController {
  ...
  openSession(sid) { this.runtime.sessions.open(sid); this.onOpen(sid); }
  async enter(id, options = {}) {
    if (!this.ready || !this.available(id)) throw Error('工作台尚未安装或正在加载');
    this.runtime.layout?.selectPanel('huaxue-workbench');
    ...
    if (sid && this.owner(sid) === id && this.runtime.sessions.list.getSnapshot().byId[sid]) this.openSession(sid);
    else {
      this.runtime.sessions.clear();
  ...
  async leave() {
    this.runtime.layout?.selectPanel(null);
    ++this.epoch;
  ...
  async create({ workbenchId = this.state.activeId, workspaceId, background = false, initialize } = {}) {
    ...
    if (!background && epoch === this.epoch && this.state.activeId === workbenchId) this.openSession(sid);
    return sid;
  }
